...
// Runs every hour on the hour; sends only when a zone's local hour == 20 (8PM)
export const sendDaily8pmByTZ = onSchedule(
  ...
  async () => {
    ...
    console.log(
      `⏰ UTC ${utcHour}:00 → daily 8PM offsets: ${due.join(",")} (target ${TARGET_LOCAL_HOUR})`
    );

    const sends = due.map(off => {
      const sign = off >= 0 ? "p" : "m";
      const absH = Math.abs(off).toString().padStart(2, "0");
      const topic = `daily8pm_tz_${sign}${absH}`;
